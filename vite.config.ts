    ...
    generateBundle(options, bundle) {
      ...
      for (const fileName in bundle) {
        ...
        if (chunk.type === 'chunk' && fileName.includes('vendor')) {
          ...
          // The pattern is: request.url.indexOf("file: (unclosed string, no closing quote)
          // Replace with: request.url.indexOf("file://") === 0) {
          // Simple string replacement - find and replace the exact broken pattern
          if (code.includes('.indexOf("file:') && !code.includes('.indexOf("file://"')) {
            // Replace the broken pattern - handle both with and without "if (" prefix
              ...
              while (lookAhead < 15 && i + lookAhead + 1 < lines.length) {
                ...
                if (nextLine && new RegExp(`\\b${storedVarName.replace(/\$/g, '\\$')}(\\.[\\w$]+|\\[|\\s*[=,;:\\)\\}])`).test(nextLine)) {
                  ...
                  if (!wrappedNext) {
                    ...
                    newLines.push(`(function(){function _waitFor${storedVarName.replace(/\$/g, '_')}(){if(!${storedVarName}){setTimeout(_waitFor${storedVarName.replace(/\$/g, '_')},0);return;}}_waitFor${storedVarName.replace(/\$/g, '_')}();`);
                    wrappedNext = true;
